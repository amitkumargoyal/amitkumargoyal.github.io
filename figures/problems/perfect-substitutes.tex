...
\newcommand{\esmark}{%
  \IfFileExists{es-logo.png}
    {\raisebox{-0.34\height}{\includegraphics[height=5mm]{es-logo.png}}}
...
During exam week a student wants only to stay awake, and treats tea and coffee as interchangeable sources of caffeine. Take one cup of coffee to carry twice the caffeine of one cup of tea, and let
\begin{itemize}
  \item $x$: cups of tea, at $p_x=4$ each;
  \item $y$: cups of coffee, at $p_y=5$ each.
\end{itemize}

Total caffeine, measured in cups-of-tea, is then $u(x,y)=x+2y$. With a weekly budget of \$20 for hot drinks, find the optimal bundle $(x^\star,y^\star)$ and illustrate the budget line and the optimal choice in a diagram.

\subheading{Solution}
The student solves
\[
  \begin{aligned}
    \max_{(x,y)\in\mathbb{R}^2_+}\quad & x+2y\\
    \text{s.t.}\quad & 4x+5y\le 20.
  \end{aligned}
\]
...
\]
Since $\MU_x/p_x < \MU_y/p_y$, every dollar buys more caffeine from coffee, so the student spends the whole budget on $y$ (coffee). With $M=20$ and $p_y=5$, the optimal bundle is
\[
...
\begin{tikzpicture}
  \begin{axis}[
    ...
    \addplot[indiffsoft, domain=0:2] {1   - 0.5*x}
      node[pos=0.5, above right=-1pt, font=\scriptsize, text=inksoft] {$u=2$};
    \addplot[indiffsoft, domain=0:5] {2.5 - 0.5*x};
    \addplot[indiff, domain=0:8] {4 - 0.5*x}
...
\[
  u(x,y)=x+y.
\]
A bus ride costs $p_x=2$ and a metro ride $p_y=3$, and the weekly transport budget is \$18.
...
\begin{enumerate}
  ...
  \item Suppose the metro authority introduces a discounted student fare, cutting the metro price to $p_y=2$. With everything else unchanged, characterise the set of utility-maximising bundles.
\end{enumerate}
...
\[
  u(x,y)=x+1.5\,y,
\]
with the original prices and budget, $p_x=2$, $p_y=3$, and $M=18$.
